...
import { loadAuthority, isWorkspaceUuid } from "./landingPageWorkspaceAuthority";
import type { Authority } from "./landingPageWorkspaceAuthority";
...
> {
  if (!isLandingPageWorkspaceEnabled()) return { ok: false, error: "disabled" };
  if (!isWorkspaceUuid(input.landingPageId)) return { ok: false, error: "not_found" };
...
export async function saveAccountLandingPageOperationalConfiguration(input: Readonly<{
  ...
  if (!authority.value.canMutate || !isWorkspaceUuid(input.landingPageId)) {
...
export async function approveAccountLandingPageRevision(input: Readonly<{
  ...
  if (
    ...
    !isWorkspaceUuid(input.landingPageId) ||
    !isWorkspaceUuid(input.materializationId)
  ) {
    return { ok: false, error: "unauthorized" };
  }
  try {
...
> {
  ...
  for (const row of data) {
    if (
      !isRecord(row) || typeof row.id !== "string" || !isWorkspaceUuid(row.id) ||
