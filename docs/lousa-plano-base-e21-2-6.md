...
- Identidade: alterar o RPC de criação por migration incremental para bloquear atomicamente nova variante de caixa na mesma modalidade, usando comparação derivada apenas para detecção e lock transacional por identidade comparada. Preservar a PK e todos os valores originais. O cadastro comum não converte caixa e não cria duplicata, mesmo sob concorrência.
- Reconciliação focal: desabilitar para novas seleções `GPT-6-Sol` e `GPT-6-luna` sem apagá-los, sem alterar seus parâmetros ou o histórico. `gpt-6-sol` já existe e permanece canônico. Como `gpt-6-luna` ainda não existe, uma operação administrativa explícita de reconciliação valida ao vivo esse ID exato na OpenAI e, atomicamente, confirma a indisponibilidade da variante antiga e cadastra a identidade canônica inicialmente indisponível, preservando o effort `xhigh` conhecido. Essa operação é exceção restrita à reconciliação histórica deste caso; não abre o cadastro comum a variantes de caixa. A disponibilização posterior do modelo e effort canônicos continua sendo ação humana explícita.
...
- Validação: `npm ci` → testes focais de action, UI e boundary → `npm run check`; migration integral em PostgreSQL compatível e testes SQL positivos/negativos por transação efêmera com rollback antes de merge. QA administrativa em Preview quando migration e credencial estiverem disponíveis no consumidor autorizado, comprovando cadastro válido/inválido, duplicata por caixa, inclusão posterior de effort e ausência de alteração automática de revisões; até lá, evidência hospedada obrigatória permanece pendente e o PR não fica pronto para merge.
