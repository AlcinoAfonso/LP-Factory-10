...
- Correção objetiva da Passagem 1: `dd64f6545cbb63efeb72f05d126ec9ac38899e9d`, blob v2 vigente `3cc9db358a8ac4b17d7c146d97cca0fe324ff6a2`.
...
| `AN-P2-E22.3-01` | `docs/automations.md` registra o Agent Builder como histórico, enquanto `docs/platform-config.md` ainda o registra como ativo operacional dependente do endpoint MCP. | invariante técnico | Impede falso negativo no gate externo por confiar em uma única classificação documental. | derivação técnica da v1 | Incorporado: reconciliar o estado externo em `E22.3.3` e manter a retirada pendente até haver evidência suficiente. | `docs/lousa-plano-base-e22-3.md` §5.2 e `E22.3.3`. | Passagem 2 e revisão delta do mesmo Analista; nenhuma correção restante. |
...
- Não avançar se surgir consumidor externo necessário sem substituto aprovado, outro workload no projeto alvo, dependência do Core/automação preservada ou necessidade material nova.

## 7. Próximo gate

A Passagem 2 `auditoria_consolidacao` foi concluída no mesmo Analista e aprovou a consolidação após a revisão delta, sem pareceres especializados. O próximo gate é aplicar o Prompt ABC ao `docs/roadmap.md`, submetendo o resultado ao mesmo gate do Analista antes de versionar `LP-Factory-Stage: plan-v2-approved`.
