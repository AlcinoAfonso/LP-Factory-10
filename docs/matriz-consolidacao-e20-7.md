...
| Gestor de Automações | VM-E20.7.4-01 | Configuração própria exige decisão material verificável. | requer validação material pelo Analista | extensão adjacente necessária e proporcional | incorporado e superado por decisão humana | N/A | 1.7; 2.3; 3.2 | O PR #837 fixou `gpt-5.6-luna + high` com search context code-owned `medium` e dispensou a matriz comparativa anterior. |
...
| HUM-E20.7-P1-05 | Fixar `landing_page_dynamic_market_research` em `gpt-5.6-luna + high`, dispensar a matriz comparativa anterior e manter `max` fora; nenhuma regra pode impedir `save` ou `promote` da combinação autorizada. | decisão operacional superveniente registrada pelo PR #837 | incorporado | v2 1.7, 2.3 e 3.2; registry, migration, fixtures e documentos canônicos |
