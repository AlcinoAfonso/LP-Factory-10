...
- Quando a configuração estiver incompleta, cada dimensão sem valor deve permanecer representada com indicação honesta equivalente a `Não informado`; quando `transaction_intent` não for aplicável, deve aparecer como `Não se aplica`. Não inventar valor, não confundir ausência com não aplicabilidade e não omitir silenciosamente uma dimensão exigida.
...
- Entrega executável:
  ...
  - rota dedicada `/a/[account]/landing-pages/new` para a criação vigente com `name` e `slug`, sem auto-geração do slug e sem expô-lo na coleção. Preservar autoridade server-side fail-closed: owner/admin ativos podem criar; viewer não recebe controle nem autoridade aparente. Em sucesso, navegar para o detalhe da LP criada; em erro, permanecer no subfluxo de criação com feedback seguro e sem persistência parcial;
  ...
  - transferir o controle `Gerar nova versão` e a propriedade route-local de sua Server Action do preview para o boundary da rota de detalhe da LP. Preservar sem alteração semântica guards, entitlement, contexto, versão-base, providers, revalidação, idempotência, materialização, deadline interno vigente e `maxDuration = 300` no novo segmento executor. Em sucesso, consumir o `revisionId` já retornado para navegar à rota existente de preview da revisão criada; em erro, permanecer no detalhe; durante pending, bloquear somente nova submissão do mesmo controle. Remover do preview os arquivos ou controles de geração que perderem função e cobrir regressão focal contra perda acidental do timeout ou de sua configuração;
