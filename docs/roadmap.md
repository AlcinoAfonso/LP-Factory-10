...
- Status: fundação, inventário, configuração operacional e catálogo administrativo vigentes; avaliação comparativa E21.3 pausada; E21.5 é a autoridade ativa do controle de custos programáticos até a visão administrativa vigente e possui a E21.5.6 planejada para completar a hierarquia econômica por evento, enquanto E21.4 preserva o total oficial e a série histórica congelada de Landing Pages. A pesquisa dinâmica permanece sem consumidor no fluxo de Landing Page.
...
- Status: entregas até E21.5.5 concluídas e vigentes como autoridade ativa do custo programático por workload e conta; E21.5.6 planejada, ainda sem implementação, apply ou QA, preservando a Costs API como autoridade do total e a E21.4 como histórico congelado.
...
- Status: planejada; V2 técnica consolidada e aprovada, com implementação, migration, apply e QA ainda não iniciados.
- Conteúdo:
  - completar na superfície `/admin/custos-openai` a hierarquia `universo → responsável econômico → evento de negócio → workload → execução/operação`, preservando gasto oficial, reconciliação, coverage, pricing e histórico E21.4;
  - tratar LP específica, ocorrência de resolução de nicho e evento interno LP Factory como identidades econômicas explícitas quando comprovadas na origem, mantendo ausência de correlação visível e sem inferência por identificador técnico ou heurística;
  - compor ativo e legado com decimal lossless, manter custo indisponível distinto de zero e usar nomes humanos como rótulo primário;
  - implementar no ledger e nos adapters existentes por extensão forward-only e RPCs v2 com ponte v1 estrita durante rollout não atômico, sem backfill, tabela analítica, novo engine, dashboard paralelo ou reabertura dos PBs concluídos.
