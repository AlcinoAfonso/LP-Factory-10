...
- Aplicar os critérios WCAG 2.2 pertinentes à superfície e registrar evidência manual de teclado, ordem e foco visível, labels/names, feedback de sucesso e erro, ausência de interação exclusiva por hover, contraste e alvos de toque; auditoria automática é apoio e não autoriza declarar conformidade WCAG 2.2 integral.
...
- Manter um único boundary em `lib/openai-workloads/`. Criar `adapters/modelCatalogAdapter.ts` e `adapters/modelCatalogAdapterCore.ts` para o novo agregado, sem incorporar sua normalização ao adapter de lifecycle.
