...
| ARC-010 | Alterado | Drift E20.7 e corte E21.4 descritos na auditoria foram reconciliados pelos documentos atuais. Persistem afirmações superadas da E20.6.5/E21.2.4 no roadmap: revisão 1 versus revisão operacional 2 efetivamente ativa nos dois ambientes. Ver 2.4; não declarar resolução global. | Registro factual AA-PR01; qualquer patch canônico exige ABC separado |
...
- ARC-010: `docs/roadmap.md` 20.6.5 e 21.2.4 ainda descrevem revisão bootstrap 1 ou promoção da revisão 2 como futura para suficiência. `docs/platform-config.md` 3.5 descreve a progressão histórica de rollout. A revisão operacional 2 ativa está comprovada no banco; isso, isoladamente, não prova flags, redeploy, QA final ou conclusão da E20.6.5. Canônicos preservados neste PR de baseline: drift registrado para avaliação, sem fechar gates não inspecionados. Qualquer correção canônica exige ABC independente por documento; nenhuma limpeza geral de histórico está autorizada.
...
- Gates de entrega: main reconfirmada no mesmo SHA; nenhum PR aberto na entrada e somente o draft AA-PR01 [#847](https://github.com/AlcinoAfonso/LP-Factory-10/pull/847) após publicação. Inventário de migrations comparado com igualdade exata; `main..HEAD`, `main...HEAD` e `git diff --check` revisados; diff acumulado restrito a esta lousa, sem secrets ou alterações de banco/workflows/runtime. ARC-012 deste PR: um arquivo documental, owner documental de consolidação, zero domínios produtivos alterados.
