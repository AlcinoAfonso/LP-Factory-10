...
| Gestor Estrutural | `GE-03` | Metadados estruturais e substituição dos caminhos vigentes estavam subespecificados. | Bloqueante com patch `PE-03`. | preservação | incorporado | N/A | 1.5, 1.6, 2.1 e 3.1 | A v2 fixa scope/origin/substitution/version, retirada forward-only, reuso de `sameCommercialWorkConfirmed` e ausência de autoridade corrente nos fields antigos. |
| Gestor Estrutural | `GE-04` | O banco vigente já suporta o caso; migration deve ser explicitamente excluída. | Bloqueante com patch `PE-04`. | preservação | incorporado | N/A | 1.7, 2.1 — Persistência e 4.2 | A v2 proíbe schema, migration, ACL e nova residência, reutilizando as residences e RPCs vigentes; divergência operacional exige parada. |
...
- Estado antes da Passagem 2: correções ainda não aplicadas; serão tratadas no mesmo Analista por `revisao_delta` após a auditoria da consolidação.
