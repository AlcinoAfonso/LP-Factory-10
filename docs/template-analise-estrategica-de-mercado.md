12/07/2026 — Template de análise estratégica de mercado

Versão: v2

0. Objetivo
Orientar pesquisas recorrentes de mercado, comparar as evidências com o estado real do LP Factory 10 e produzir decisões práticas de posicionamento, produto e execução.

1. Recorte da análise
1.1 Tema
• [mercado, tecnologia, comportamento, concorrência ou problema do cliente]

1.2 Pergunta estratégica
• [decisão que a pesquisa deve apoiar]

1.3 Horizonte
• [agora | próximos 6 meses | próximos 12 meses]

2. Fontes obrigatórias
• Documentação interna aplicável: README.md, docs/roadmap.md, docs/base-tecnica.md quando houver impacto técnico e documentos específicos do recorte.
• GitHub: código, estrutura, convenções e estado implementado.
• Mercado externo: fontes atuais, identificadas por nome e data.
• Regra de leitura: interpretar as fontes sem reproduzi-las e separar maturidade técnica de valor e diferenciação de mercado.

3. Evidências de mercado
3.1 Movimento observado
• [mudança relevante]

3.2 Problema do cliente
• [custo, tempo, risco, assertividade, receita ou operação]

3.3 Soluções e concorrentes
• [solução ou concorrente] — [proposta] — [evidência]

3.4 Velocidade e maturidade
• [emergente | acelerando | consolidando]
• [sinais que sustentam a classificação]

4. Comparação com o LP Factory 10
4.1 Estado atual
• [capacidade, decisão ou entrega existente] — [fonte ou path]

4.2 Aderência ao mercado
• [forte | parcial | fraca] — [justificativa curta]

4.3 Diferenciais aproveitáveis
• [ativo real do projeto que pode gerar vantagem]

4.4 Lacunas relevantes
• [lacuna comprovada pela comparação]

5. Diagnóstico estratégico
5.1 Oportunidades
• [oportunidade] — [impacto esperado]

5.2 Riscos
• [risco] — [probabilidade e efeito]

5.3 Trade-offs
• [opção A versus opção B] — [critério de decisão]

6. Rotas possíveis
6.1 Manter
• [decisão ou direção]

6.2 Ajustar
• [mudança incremental]

6.3 Acelerar
• [ação com urgência comprovada]

6.4 Adiar
• [item que pode aguardar]

6.5 Cortar
• [item que perdeu valor estratégico]

6.6 Investigar
• [hipótese sem evidência suficiente]

7. Recomendação
7.1 Direção recomendada
• [decisão principal]

7.2 Justificativa
• [evidências mais relevantes]

7.3 Efeito esperado
• [diferenciação, custo, velocidade, assertividade ou receita]

8. Plano de ação
8.1 Ação imediata
• [ação] — [responsável] — [prazo ou gatilho]

8.2 Ajustes documentais
• [arquivo e seção] | N/A

8.3 Ajustes de produto ou execução
• [caso, fase ou entrega] | N/A

8.4 Validação
• [métrica, teste ou evidência esperada]

9. Decisões congeladas
• [decisão] — [data] — [condição para reavaliação]

10. Síntese executiva
• Movimento de mercado: [resumo]
• Situação do LP Factory 10: [resumo]
• Decisão recomendada: [resumo]
• Próxima ação: [resumo]
