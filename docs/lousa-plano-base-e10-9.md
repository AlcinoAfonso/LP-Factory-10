...
- O runtime executa matching e confiança determinísticos antes de qualquer chamada. Alta confiança suficientemente única encerra sem OpenAI. Ambiguidade pode gerar no máximo uma chamada foreground por turno e três chamadas OpenAI por Pending Setup. A conversa persiste `openai_call_count` com limite 0–3; uma RPC transacional, vinculada à reserva exata do turno, incrementa e devolve esse contador imediatamente antes de toda chamada ao provedor. A contagem independe de append, reload, retry e ledger financeiro; o caminho determinístico não incrementa. Se a terceira chamada não produzir resolução segura, o runtime encerra a investigação de nicho naquele fluxo, não formula nova pergunta de nicho e transita para confirmação do fallback operacional com a mensagem exata aprovada.
...
- A resolução de nicho faz no máximo três chamadas OpenAI por Pending Setup, comprovadas pelo contador autoritativo persistido na própria conversa e reservado antes do provedor, independentemente do append e do ledger financeiro; se a terceira não produzir resolução segura, o turno persiste e apresenta exatamente “Ainda não consegui identificar seu nicho com segurança. Vou preservar o que você me contou para seguirmos sem associar uma categoria incorreta.”, não apresenta nova pergunta de nicho e transita para o fallback operacional irreversível. Falha ao confirmar esse fallback permite somente repetir sua confirmação, sem reabrir investigação, automação, follow-up, canal preferido ou infraestrutura adicional.
