...
Definir a arquitetura única que conduz um caso ou recorte desde o Debate até sua conclusão, com o menor processo proporcional ao risco e uma única residência para cada regra.
...
Conflito entre fontes deve ser devolvido ao supervisor competente. Um contrato do Pipeline não pode substituir unilateralmente regra vigente do `AGENTS.md` ou fonte canônica especializada.
...
- Updates é obrigatório somente quando capacidades atuais de plataforma ou tecnologia puderem alterar materialmente a solução, a qualidade ou a validação.
- Quando previsível, o Debate registra a participação de Updates; quando descoberta durante a implementação, o Executor pode chamá-lo diretamente.
- Gestor Estrutural pode confirmar que boundaries, autoridades e arquitetura existentes são suficientes; necessidade de desenho estrutural material novo obriga Completo.
- Gestor de Automações pode atuar quando o recorte permanecer dentro de automação, autoridade e lifecycle existentes; novidade material nesses pontos obriga Completo.
- Analista entra somente diante de dúvida, risco ou materialidade.
- O Executor pode consultar diretamente o especialista pertinente e retorna ao supervisor quando o parecer exigir reclassificação, mudança funcional ou decisão fora de sua autoridade.
...
- Gestor de Automações participa quando `Automação: sim`.
...
- A função atual da matriz deve ser preservada.
- A direção aprovada é substituí-la por Registro de Consolidação mínimo no workflow/PR, somente após validação de rastreabilidade equivalente ou superior.
...
- Os gates atuais do Analista por subseção permanecem nesta reorganização; redução futura é otimização separada.
...
- Manual e Semiautomático exigem autorização humana explícita para o merge final; o Estrategista original pode executá-lo remotamente quando essa autoridade estiver materializada no `AGENTS.md`.
- Autônomo permite merge pelo Estrategista Autônomo quando gates, checks, QA e evidências obrigatórios estiverem satisfeitos e não houver decisão material pendente, após essa autoridade ser materializada no `AGENTS.md`.
- Qualquer exceção material bloqueia o merge e exige escalada.
- A autoridade operacional vigente é sempre a que estiver materializada no `AGENTS.md`.
...
- `lp-factory-executar-plano`: subfluxo específico da implementação Completa, subordinado ao Prompt Executor universal após sua migração.
...
- Cada etapa deve preservar um pipeline coerente e utilizável.
- Compatibilidades necessárias entram no mesmo PR que muda o contrato correspondente.
- O comportamento operacional ainda não migrado permanece regido pelo contrato vigente no repositório.
- Não se remove arquivo, gate ou controle antes de sua responsabilidade possuir residência e o fluxo representativo estar validado.
- O plano e a ordem da migração permanecem registrados no Debate 3 — Pipeline de Plano Base.
