...
1.23 real_estate_lab_contents
1.23.1 Chaves, constraints e relacionamentos
• PK: id uuid default gen_random_uuid()
• FK: account_id → accounts(id) ON UPDATE CASCADE ON DELETE CASCADE
• FK: created_by → auth.users(id) ON UPDATE CASCADE ON DELETE RESTRICT
• FK: updated_by → auth.users(id) ON UPDATE CASCADE ON DELETE RESTRICT
• CHECK: status IN ('idea','draft','review','approved','published','archived')
• CHECK: channel IN ('instagram','tiktok','whatsapp','multi_channel','other')
• CHECK: title/topic não vazios após trim; body sem HTML bruto simples (`<`/`>`)
1.23.2 Campos
• title, topic, channel, format, hook, body, cta, status, scheduled_for, published_at, result_notes, learning_notes, created_by, updated_by, created_at, updated_at
1.23.3 Índices
• real_estate_lab_contents_account_status_idx (account_id, status)
• real_estate_lab_contents_account_created_at_idx (account_id, created_at DESC)
• real_estate_lab_contents_account_scheduled_for_idx (account_id, scheduled_for)
1.23.4 Segurança
• RLS ativo; grants SELECT/INSERT/UPDATE/DELETE para authenticated.
• SELECT: is_platform_admin() OU membro ativo da conta.
• INSERT/UPDATE: is_platform_admin() OU owner/admin/editor ativo; viewer não escreve; created_by/updated_by devem corresponder a auth.uid() no INSERT e updated_by a auth.uid() no UPDATE.
• DELETE: is_platform_admin() OU owner/admin ativo.
• Trigger real_estate_lab_contents_set_updated_at executa public.tg_set_updated_at().

1.24 real_estate_lab_items
1.24.1 Chaves, constraints e relacionamentos
• PK: id uuid default gen_random_uuid()
• FK: account_id → accounts(id) ON UPDATE CASCADE ON DELETE CASCADE
• FK: responsible_user_id → auth.users(id) ON UPDATE CASCADE ON DELETE SET NULL
• FK: created_by → auth.users(id) ON UPDATE CASCADE ON DELETE RESTRICT
• FK: updated_by → auth.users(id) ON UPDATE CASCADE ON DELETE RESTRICT
• CHECK: item_type IN ('agenda','task','decision','question','objection','learning','opportunity','field_note')
• CHECK: status IN ('open','in_progress','done','discarded')
• CHECK: priority NULL ou IN ('low','medium','high')
• CHECK: title não vazio após trim; description sem HTML bruto simples (`<`/`>`)
1.24.2 Campos
• item_type, title, description, status, priority, responsible_user_id, due_at, occurred_at, result_notes, created_by, updated_by, created_at, updated_at
1.24.3 Índices
• real_estate_lab_items_account_item_type_idx (account_id, item_type)
• real_estate_lab_items_account_status_idx (account_id, status)
• real_estate_lab_items_account_due_at_idx (account_id, due_at)
• real_estate_lab_items_account_created_at_idx (account_id, created_at DESC)
1.24.4 Segurança
• RLS ativo; grants SELECT/INSERT/UPDATE/DELETE para authenticated.
• SELECT: is_platform_admin() OU membro ativo da conta.
• INSERT/UPDATE: is_platform_admin() OU owner/admin/editor ativo; viewer não escreve; created_by/updated_by devem corresponder a auth.uid() no INSERT e updated_by a auth.uid() no UPDATE.
• DELETE: is_platform_admin() OU owner/admin ativo.
• Trigger real_estate_lab_items_set_updated_at executa public.tg_set_updated_at().
