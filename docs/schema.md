...
• A revisão bootstrap `1` do novo workload usa `gpt-5.6-luna + low` somente como origem repo_catalog; não comprova nem autoriza transporte hospedado, que exige revisão operacional `2` ou posterior.
• RLS habilitado e nenhuma policy.
• public, anon, authenticated e ai_readonly: sem grants.
...
99. Changelog
v1.0.60 (29/08/2026) — E20.7.4: registrada a migration candidata que amplia o agregado E21.2 de dez para doze unidades com `landing_page_dynamic_market_research`, preserva as três tabelas, RLS e grants existentes e mantém apply e revisão operacional comprovada pendentes do merge humano.
