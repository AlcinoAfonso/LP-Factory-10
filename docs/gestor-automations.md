...
* Quando a solução com OpenAI depender de prompt consumido no runtime, consultar `docs/template-prompts.md` e o complemento específico do modelo quando existir e for aplicável, atualmente `docs/template-prompts-gpt-5-6.md` para GPT-5.6. Verificar no plano que o prompt será versionado em código, separará instruções da aplicação de input/contexto dinâmico, usará valores tipados ou validados, respeitará o contrato de saída e terá casos representativos de validação antes de promoção.
...
* Prompt de runtime, quando aplicável: templates consultados, estratégia de versionamento, fronteira entre instruções e contexto dinâmico, contrato de entrada/saída e validação representativa; o parecer não precisa conter o prompt final salvo quando isso fizer parte explícita do escopo.
