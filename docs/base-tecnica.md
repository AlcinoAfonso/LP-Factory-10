...
1. [Identificação do Projeto](#1-identificação-do-projeto)  
   - [1.1 Variáveis Obrigatórias (server-only)](#11-variáveis-obrigatórias-server-only)  
   - [1.2 Variáveis Públicas](#12-variáveis-públicas)  
...
2. [Stack & Dependências](#2-stack--dependências)  
   ...
   - [2.5 Regras de Import](#25-regras-de-import)  

3. [Regras Técnicas Globais](#3-regras-técnicas-globais)  
   - [3.1 Segurança](#31-segurança)  
   - [3.2 Estrutura de Camadas](#32-estrutura-de-camadas)  
   - [3.3 Estrutura de Arquivos](#33-estrutura-de-arquivos)  
   - [3.4 CI/Lint (Bloqueios)](#34-cilint-bloqueios)  
   - [3.5 Secrets & Variáveis](#35-secrets--variáveis)  
   - [3.6 Tipos TypeScript](#36-tipos-typescript)  
   - [3.7 Convenções](#37-convenções)  
   - [3.8 Anti-Regressão](#38-anti-regressão)  
   - [3.9 Rate Limit (E7)](#39-rate-limit-e7)  
   - [3.10 Anti-Patterns](#310-anti-patterns)  
   - [3.11 Sistema de Grants (E9)](#311-sistema-de-grants-e9)  
   - [3.12 Compatibilidade PostgREST 13](#312-compatibilidade-postgrest-13)  

4. [Schema](#4-schema)  
   - [4.1 Tabelas Ativas](#41-tabelas-ativas)  
   - [4.2 Views Ativas](#42-views-ativas)  
   - [4.3 Functions Ativas](#43-functions-ativas)  
   - [4.4 Triggers Ativos](#44-triggers-ativos)  
   - [4.5 Tipos TypeScript Canônicos](#45-tipos-typescript-canônicos)  

5. [Arquitetura de Acesso](#5-arquitetura-de-acesso)  
   - [5.1 Conceitos Fundamentais](#51-conceitos-fundamentais)  
   - [5.2 Adapters, Guards, Providers](#52-adapters-guards-providers)  
   - [5.3 Fluxos Principais](#53-fluxos-principais)  

6. [Estrutura de Arquivos Essencial](#6-estrutura-de-arquivos-essencial)  
   - [6.1 Estrutura Base por Domínio](#61-estrutura-base-por-domínio)  
   - [6.2 Arquivos Críticos](#62-arquivos-críticos)  
   - [6.3 Tipos e Contratos Críticos](#63-tipos-e-contratos-críticos)  
   - [6.4 Arquivos SULB Autorizados](#64-arquivos-sulb-autorizados-a-importar-supabase)  
   - [6.5 Regras Rápidas](#65-regras-rápidas)  

7. [Checklists Declarativos](#7-checklists-declarativos)
...
- **Campos-chave:**  
  `account_id`, `account_key` (subdomain), `account_name`,  
  `account_status`, `user_id`, `member_role`, `member_status`,  
  `allow`, `reason`
...
- **Campos principais:**  
  `account_id`, `account_name`, `account_status`, `subdomain`,  
  `domain`, `plan_id`, `plan_name`, `price_monthly`, `plan_features`,  
...
- **Campos principais:**  
  `token_id`, `email`, `expires_at`, `is_used`, `is_valid`,  
  `account_slug`, `created_at`
...
- **Campos principais:**  
  `id`, `entity`, `entity_id`, `action`, `diff`,  
  `account_id`, `actor_user_id`, `ip_address`, `created_at`
...
- Conjunto antigo de triggers permanece apenas para rollback  
...
- Aplica rate limit sobre `post_sale_tokens` (ver seção 3.9).
...
Regras:
- Fluxo sempre passa pela SULB; não criar formulários auth fora deste núcleo.  
- Após login/reset/update, o redirect final deve passar pelo Access Context (`/a/[account]` ou `/auth/confirm/info`).
...
3. Definição de senha + criação de usuário.  
...
3. Ao trocar de conta, a UI dispara eventos de telemetria e navega para `/a/[account]`.  
...
Regra:
- Eventos devem seguir o formato padrão:  
  `{ event, scope, latency_ms, timestamp, error? }`  
