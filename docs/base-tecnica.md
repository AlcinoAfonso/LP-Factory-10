...
• Preparar publicação comprova a coleção integral afetada sem alterar atividade ou marcador factual. Após o deploy publicar o conteúdo repo-only exato, a reconciliação revalida a mesma identidade, atualiza todos os marcadores, ativa somente revisões `release` e exclui o singleton sem permitir sucesso parcial.
...
• A avaliação v2 apresenta estratégia, estado e fontes, mas aceita somente decisão humana completa: zero, parcial ou total, camada explícita por aceitação e candidato próprio marcado como humano. Mudança exige draft e revisão exatos e nunca materializa field automaticamente.
