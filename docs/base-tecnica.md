...
• A evolução administrativa do singleton usa operações puras e discriminadas `add | change | retire`, sempre com alvo/camada explícitos e contrato humano completo. `add` materializa origem e `createdInVersion`; `change` preserva chave, posição e histórico; `retire` preserva o field e registra somente `retiredInVersion` forward-only.
...
• O lifecycle administrativo mantém uma única sessão `release | revision` não encerrada por taxon, ancora baseline, conteúdo e contexto por fingerprints e por snapshot canônico da identidade completa da cadeia taxonômica, revalidada sob locks em ordem estável, e aplica concorrência otimista; drift torna decisão ou fechamento stale e preserva o estado anterior.
...
• `landing_page_input_catalog_drafts.taxon_review_evidence` é projeção RPC-only da decisão autoritativa por sessão e draft exato, contendo somente identidade da sessão e do evento, revisão e fingerprints. Editar o conteúdo limpa toda a projeção, registra invalidação e reabre atomicamente as sessões vinculadas.
