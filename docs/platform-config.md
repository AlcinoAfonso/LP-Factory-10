...
• Workload repo-side pendente de rollout: `landing_page_dynamic_market_research`, com baseline de Development `gpt-5.6-luna + low`, uma requisição foreground à Responses API, somente Web Search hospedado, Structured Output estrito, `store:false`, uma ou duas chamadas de busca e deadline máximo de 45 s.
