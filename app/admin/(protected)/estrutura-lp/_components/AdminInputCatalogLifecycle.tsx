...
function TextField({ hint = "", id, label, name, required = false }: Readonly<{ hint?: string; id: string; label: string; name: string; required?: boolean }>) {
  return <div className="mt-4 min-w-0"><label htmlFor={id} className="text-sm font-semibold text-foreground">{label}</label>{hint ? <p id={`${id}-hint`} className="mt-1 break-words text-xs text-muted-foreground">{hint}</p> : null}<input aria-describedby={hint ? `${id}-hint` : undefined} id={id} name={name} required={required} className="mt-2 min-h-11 w-full min-w-0 rounded-md border border-border bg-background px-3 text-sm outline-none focus-visible:ring-4 focus-visible:ring-brand-600/20" /></div>;
}
