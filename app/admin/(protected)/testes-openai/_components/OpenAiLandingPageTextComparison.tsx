...
type EvaluationDraft = LandingPageDraftComparisonEvaluation;
...
function ResultCard({
  ...
  return (
    <article className="min-w-0 rounded-lg border border-border bg-background p-4 sm:p-5">
      ...
      {result.attempt.ok ? <fieldset className="mt-5 space-y-4 border-t border-border pt-4" disabled={disabled || revealed}><legend className="text-sm font-semibold text-foreground">Régua humana</legend><div className="grid gap-3 sm:grid-cols-3"><div><label htmlFor={`${fieldId}-validity`} className="text-xs font-medium text-foreground">Validade</label><select id={`${fieldId}-validity`} className={field} value={evaluation.validity} onChange={(event) => onEvaluationChange({ validity: event.target.value as EvaluationDraft["validity"] })}><option value="valid">Válido</option><option value="invalid">Inválido</option></select></div><div><label htmlFor={`${fieldId}-quality`} className="text-xs font-medium text-foreground">Qualidade</label><select id={`${fieldId}-quality`} className={field} value={evaluation.quality} onChange={(event) => onEvaluationChange({ quality: event.target.value as EvaluationDraft["quality"] })}><option value="insufficient">Insuficiente</option><option value="adequate">Adequada</option><option value="superior">Superior</option></select></div><div><label htmlFor={`${fieldId}-correction`} className="text-xs font-medium text-foreground">Correção humana</label><select id={`${fieldId}-correction`} className={field} value={evaluation.correction} onChange={(event) => onEvaluationChange({ correction: event.target.value as EvaluationDraft["correction"] })}><option value="none">Nenhuma</option><option value="light">Leve</option><option value="relevant">Relevante</option></select></div></div><div><label htmlFor={`${fieldId}-comment`} className="text-xs font-medium text-foreground">Comentário opcional</label><textarea id={`${fieldId}-comment`} className={`${field} min-h-20`} maxLength={800} value={evaluation.comment} onChange={(event) => onEvaluationChange({ comment: event.target.value })} /></div><button type="button" className={secondaryButton} onClick={onRegister}>{evaluationRegistered ? "Avaliação registrada" : "Registrar avaliação"}</button></fieldset> : null}
...
function initialEvaluation(): EvaluationDraft {
  return { validity: "valid", quality: "adequate", correction: "none", comment: "" };
}
