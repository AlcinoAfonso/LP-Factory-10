      ...
      <main className="mx-auto max-w-6xl px-4 pb-2 pt-6 sm:px-6 sm:pt-10">
        ...
        <section className="mt-4 rounded-2xl border border-surface-border bg-white p-5 shadow-card sm:p-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            ...
            <form action={setLandingPageArchivedAction}><input type="hidden" name="account" value={accountSubdomain}/><input type="hidden" name="landing_page_id" value={landingPageId}/><input type="hidden" name="archived" value={archived?"0":"1"}/><button className="min-h-11 rounded-lg border border-surface-border bg-white px-4 text-sm font-semibold text-ink-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600">{archived?"Restaurar página":"Arquivar página"}</button></form>
          </div>
        ...
        <section className="mt-6 rounded-2xl border border-surface-border bg-white p-5 shadow-card sm:p-7">
          ...
          <div className="mt-5 grid gap-3">
            {detail.revisions.length===0?<p className="rounded-lg border border-dashed border-surface-border p-5 text-sm text-graytech-600">Nenhuma versão gerada.</p>:detail.revisions.map((revision)=><article key={revision.id} className="flex flex-col gap-3 rounded-xl border border-surface-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-ink-900">Versão {revision.number}{revision.latest?" · mais recente":""}{revision.approved?" · aprovada":""}</p><p className="mt-1 text-xs text-graytech-600">{new Date(revision.createdAt).toLocaleString("pt-BR")}</p></div><div className="flex flex-wrap gap-2"><Link href={`/a/${accountSubdomain}/landing-pages/${landingPageId}/preview?revision=${revision.id}`} className="inline-flex min-h-11 items-center rounded-lg border border-surface-border px-4 text-sm font-semibold text-ink-900">Visualizar</Link>{!archived&&!revision.approved?<form action={approveLandingPageRevisionAction}><input type="hidden" name="account" value={accountSubdomain}/><input type="hidden" name="landing_page_id" value={landingPageId}/><input type="hidden" name="materialization_id" value={revision.id}/><button className="min-h-11 rounded-lg bg-brand-700 px-4 text-sm font-semibold text-white">Aprovar esta versão</button></form>:null}</div></article>)}
          </div>
        </section>
      </main>
      {archived ? <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6"><section className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><h2 className="font-bold text-amber-950">Página arquivada</h2><p className="mt-2 text-sm text-amber-900">O histórico e a aprovação permanecem visíveis. Restaure a página para salvar configuração, gerar ou aprovar.</p></section></main> : <OnboardingConfigurationJourney accountSubdomain={accountSubdomain} configuration={detail.configuration.resolved} workspaceMode sharedRevision={detail.configuration.sharedRevision}/>} 
    </>
  );
}
