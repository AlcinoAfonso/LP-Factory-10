...
export function GenericCommercialPage({ accountSubdomain }: Props) {
  ...
  useEffect(() => {
    void trackCommercialEvent({ accountSubdomain, event: 'commercial_page_view' });
  }, [accountSubdomain]);

  const trackPrimary = (ctaLocation: PrimaryCtaLocation) => {
  ...
  const handleFinalCtaClick = async (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    window.location.assign(content.finalCta.href);
  };
  ...
  return (
    ...
    >
      ...
      <PageSection eyebrow="Serviços" title="Uma entrega organizada do conteúdo à publicação" muted>
      ...
      <PageSection
        eyebrow="Medição"
        title="Base para aprender com os cliques reais"
        description="Além de publicar a página, a LP Factory prepara eventos essenciais para medir interesse nos CTAs e orientar melhorias futuras com base no comportamento real dos visitantes. Esse tracking cria a base para aprendizado e futuros testes A/B, sem prometer dashboard analítico, otimização automática ou testes já ativos nesta entrega."
      >
        <div />
      </PageSection>
      ...
      <PageSection id="como-funciona" eyebrow="Como funciona" title="Quatro passos até a publicação" centered>
      ...
      <section className="bg-brand-50 px-6 py-12 text-center sm:px-10 lg:px-14">
        ...
        <a href={content.finalCta.href} onClick={handleFinalCtaClick} className={cn(primaryButton, 'mt-7')}>
