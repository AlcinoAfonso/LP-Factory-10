...
- recurso conectado identificado e expressamente autorizado pela V1 pode ser consumido diretamente pelo Executor para a finalidade aprovada, sem duplicar seu conteúdo no repositório nem convertê-lo em nova infraestrutura.
...
- participação humana pequena, condicional e explicitamente aprovada pela V1 — como bootstrap inicial, renovação de sessão ou MFA/CAPTCHA insolúvel — não caracteriza falha de autonomia nem cria obrigação de automatizá-la; continua proibida dependência humana rotineira por execução quando recurso autorizado puder executar o passo;
