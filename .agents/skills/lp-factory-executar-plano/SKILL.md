...
12. tratar `requer nova rodada especializada` suspendendo somente o ponto afetado e devolvendo-o ao workflow para `revisao_focal_implementacao` do Gestor Estrutural; preservar a mesma task, branch, PR, v1 e checkpoints não afetados, e, após o retorno do workflow e a aprovação do delta pelo Analista, retomar pela subseção correspondente ao checkpoint afetado mais antigo indicado pela revisão focal e reexecutar em ordem os checkpoints afetados e seus gates; se nenhum checkpoint anterior tiver sido afetado, retomar a subseção originalmente suspensa;
