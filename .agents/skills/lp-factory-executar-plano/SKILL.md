...
- recurso conectado identificado e expressamente autorizado pela V1, ou declarado em `docs/platform-config.md` como capacidade institucional transversal de QA, e não restringido pelo contrato técnico aplicável pode ser consumido diretamente pelo Executor para a finalidade aprovada, sem duplicar seu conteúdo no repositório nem convertê-lo em nova infraestrutura.
