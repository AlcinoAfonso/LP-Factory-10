...
4. Antes do gate, identificar os documentos canônicos potencialmente afetados. Para cada documento, preparar um relatório factual da implementação, preservar seu snapshot anterior, executar `$lp-factory-abc` de forma independente e aplicar somente as operações literais emitidas. Se o resultado for `SEM ALTERAÇÕES NECESSÁRIAS`, não editar o documento. Documento canônico nunca é editado diretamente durante a implementação.
...
3. Atualizar o PR com checkpoints, arquivos, validações, decisão sobre teste humano, matriz, pendências e, por documento, os ABCs executados e o resultado `delta aplicado` ou `SEM ALTERAÇÕES NECESSÁRIAS`; declarar a entrega completa e parar.
