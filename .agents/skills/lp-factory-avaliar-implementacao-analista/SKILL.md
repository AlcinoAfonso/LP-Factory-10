...
description: Avaliar uma subseção implementada ou a entrega final de uma v2 técnica aprovada do LP Factory 10 com o custom agent analista em modo read-only. Usar antes de avançar para a próxima subseção, de abrir o gate de teste humano ou de liberar o PR final para merge.
---
...
1. Confirmar repositório, worktree, branch, estado Git, SHA da v2 técnica aprovada e identificador exato da subseção.
...
4. Parar se plano, fase, diff ou evidência forem ambíguos; não reconstruir o escopo nem o contrato técnico por inferência.
...
3. Exigir que o Analista confira se o Executor implementou a v2 técnica aprovada sem redesenho unilateral, ampliação funcional, modernização nova não aprovada ou refatoração histórica ampla. Quando adapters/boundaries forem tocados, verificar coesão e aderência à extração focal prevista.
4. Quando houver documento canônico, auditar se o diff corresponde somente às operações emitidas pelo ABC e se `SEM ALTERAÇÕES NECESSÁRIAS` preservou o documento. Não refazer os critérios internos do contrato canônico.
5. Preservar a resposta integral e o estado Git antes e depois da delegação.
